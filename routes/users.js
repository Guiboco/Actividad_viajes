...
router.post('/resetPass2', async (req, res) => { // se crea una ruta de POST en donde se envía la orden de buscar la contraseña y sustituirla con un split. Primero se redirigirá al usuario al archivo .hbs que se ponga al principio (en este caso '/resetPass2') y luego se realiza la callback en donde se creará toda la lógica para cambiar la contraseña.

  try {
    const token = req.headers.referer.split('/')[5];/* se crea la token para buscar la contraseña. Esta formado por la req(require).headers(el primer fragmento del jwt, es decir, lo que está antes del primer punto).referer*/
